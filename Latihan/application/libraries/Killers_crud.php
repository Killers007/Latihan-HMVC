...
class Killers_crud {
	...
	public function set_md5($name=FALSE)
	{
		$this->md5=$name;
		return $this->md5_status=TRUE;
	}
	...
	{
		$this->table=$str;
		$this->field=$this->CI->db->get($this->table)->field_data();
	...
	public function add_data($data){
		...
		if ($this->md5_status) {
			$data[$this->md5]=md5($data[$this->md5]);
		}
	...
	public function update_data($data){
		...
		if($this->CI->form_validation->run()== FALSE){
			...
			$this->CI->db->where($this->primary_key,$data['primary']);
			unset($data['primary']);
