...
class Grocery_crud_model  extends CI_Model  {
    ...
    {
        parent::__construct();
        $this->db2=$this->load->database('grocery',TRUE);
    }

    function db_table_exists($table_name = null)
    {
    	return $this->db2->table_exists($table_name);
    }
    ...
    function get_list()
    {
    	...
    	$results = $this->db2->get($this->table_name)->result();
    ...
    public function get_row($table_name = null)
    {
    	...
    	return $this->db2->get($table_name)->row();
    }
    ...
    {
    	$this->db2->where( $key, $value, $escape);
    }

    function or_where($key, $value = NULL, $escape = TRUE)
    {
    	$this->db2->or_where( $key, $value, $escape);
    }

    function having($key, $value = NULL, $escape = TRUE)
    {
    	$this->db2->having( $key, $value, $escape);
    }

    function or_having($key, $value = NULL, $escape = TRUE)
    {
    	$this->db2->or_having( $key, $value, $escape);
    }

    function like($field, $match = '', $side = 'both')
    {
    	$this->db2->like($field, $match, $side);
    }

    function or_like($field, $match = '', $side = 'both')
    {
    	$this->db2->or_like($field, $match, $side);
    }

    function limit($value, $offset = '')
    {
    	$this->db2->limit( $value , $offset );
    }
    ...
    function get_total_results()
    {
    	...
    	if(!empty($this->relation_n_n))
    	{
            ...
            $this->db2->select($this->table_name . '.' . $key);
        }
        
        return $this->db2->get($this->table_name)->num_rows();
    }

    function set_basic_table($table_name = null)
    {
    	if( !($this->db2->table_exists($table_name)) )
    		return false;
    ...
    function get_edit_values($primary_key_value)
    {
    	...
    	$this->db2->where($primary_key_field,$primary_key_value);
    	$result = $this->db2->get($this->table_name)->row();
    	return $result;
    }
    ...
    function join_relation($field_name , $related_table , $related_field_title)
    {
		...
		if($related_primary_key !== false)
		{
			...
			$this->db2->join( $related_table.' as '.$unique_name , "$unique_name.$related_primary_key = {$this->table_name}.$field_name",'left');
    ...
    function get_relation_array($field_name , $related_table , $related_field_title, $where_clause, $order_by, $limit = null, $search_like = null)
    {
    	...
    	if($search_like !== null)
    		$this->db2->having("$field_name_hash LIKE '%".$this->db2->escape_like_str($search_like)."%'");

    	$order_by !== null
    		? $this->db2->order_by($order_by)
    		: $this->db2->order_by($field_name_hash);

    	$results = $this->db2->get($related_table)->result();
    ...
    function get_relation_total_rows($field_name , $related_table , $related_field_title, $where_clause)
    {
    	...
    	return $this->db2->count_all_results($related_table);
    }
    ...
    function get_relation_n_n_selection_array($primary_key_value, $field_info)
    {
    	...
    	{
    		if(!$use_template){
    			$this->db2->order_by("{$field_info->selection_table}.{$field_info->title_field_selection_table}");
    		}
    	}
    	else
    	{
    		$this->db2->order_by("{$field_info->relation_table}.{$field_info->priority_field_relation_table}");
    	}
    	$this->db2->where($field_info->primary_key_alias_to_this_table, $primary_key_value);
    	$this->db2->join(
    	...
    	$results = $this->db2->get($field_info->relation_table)->result();
    ...
    function get_relation_n_n_unselected_array($field_info, $selected_values)
    {
    	...
    	if($use_where_clause){
    		$this->db2->where($field_info->where_clause);
    	}

    	$selection_primary_key = $this->get_primary_key($field_info->selection_table);
        if(!$use_template)
        	$this->db2->order_by("{$field_info->selection_table}.{$field_info->title_field_selection_table}");
        $results = $this->db2->get($field_info->selection_table)->result();
    ...
    {
    	$this->db2->where($field_info->primary_key_alias_to_this_table, $main_primary_key);
    	if(!empty($post_data))
    		$this->db2->where_not_in($field_info->primary_key_alias_to_selection_table , $post_data);
    	$this->db2->delete($field_info->relation_table);
    	...
    	if(!empty($post_data))
    	{
    		foreach($post_data as $primary_key_value)
	    	{
				...
				$count = $this->db2->from($field_info->relation_table)->count_all_results();
				...
				if($count == 0)
				{
					...
					$this->db2->insert($field_info->relation_table, $where_array);

				}elseif($count >= 1 && !empty($field_info->priority_field_relation_table))
				{
					$this->db2->update( $field_info->relation_table, array($field_info->priority_field_relation_table => $counter) , $where_array);
				}
    ...
    function get_field_types_basic_table()
    {
    	...
    	foreach($this->db2->query("SHOW COLUMNS FROM `{$this->table_name}`")->result() as $db_field_type)
    	{
    	...
    	$results = $this->db2->field_data($this->table_name);
    ...
    {
    	$results = $this->db2->field_data($table_name);
    ...
    {
    	$primary_key_field = $this->get_primary_key();
    	return $this->db2->update($this->table_name,$post_array, array( $primary_key_field => $primary_key_value));
    }

    function db_insert($post_array)
    {
    	$insert = $this->db2->insert($this->table_name,$post_array);
    	if($insert)
    	{
    ...
    function db_delete($primary_key_value)
    {
    	...
    	$this->db2->limit(1);
    	$this->db2->delete($this->table_name,array( $primary_key_field => $primary_key_value));
    ...
    {
    	if( $this->db2->update($this->table_name,array($field_name => ''),array($field_name => $filename)) )
    	{
    		return true;
    	}
    	else
    ...
    function field_exists($field,$table_name = null)
    {
    	...
    	return $this->db2->field_exists($field,$table_name);
    }
    ...
    {
    	return $this->db2->escape_str($value);
    }
