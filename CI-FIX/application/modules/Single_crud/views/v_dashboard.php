...
<script type="text/javascript">
 ...
 function buat_input(){
  $.ajax({
    url: '<?php echo base_url('single_crud') ?>/buat_input',
    type: 'POST',
    dataType: 'json',
  })
    ...
    for(i=0;i<response.length;i++){
      <?php if (!empty($relasi->relasi)): ?>
       ...
       for(var a=0;a<relasi.length;a++){
        ...
        $.ajax({
          async : false,
          url: '<?php echo base_url('single_crud/get_relasi') ?>',
          type: 'POST',
          data: data,
...
function load_data(){
 ...
 $.ajax({
  url: '<?php echo base_url('single_crud').'/load_data' ?>',
  type: 'POST',
})
 .done(function(response) {
...
function ajax(aksi,data,status,modal){
 $.ajax({
  url: '<?php echo base_url('single_crud'); ?>/'+aksi,
  type: 'POST',
  data:data,
})
 .done(function(response) {
...
$(document).on('click', '.detail', function(){
 $.ajax({
  url: '<?php echo base_url('single_crud') ?>/detail',
  type: 'POST',
  dataType: 'json',
  data: {
   'id' : $(this).val()
...
$(document).on('click', '.delete', function(){
 ...
 swal({
  ...
  if (isConfirm) {
   $.ajax({
    url: '<?php echo base_url('single_crud').'/hapus_data' ?>',
    type: 'POST',
    data:{
     'id' : data,
   },
